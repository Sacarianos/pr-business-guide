...
**Resolved, 2026-08-24:** ported the sourcing marks past where they'd landed (entity table, patente estimate) to step and incentive cards too, via a `sourcingMark()` helper (`src/scripts/tool.ts`) shared with the pre-existing one. `practice.contradicted` got the original design work the ticket called for, since the handoff had none: a filled banner (`practiceBanner()` in `src/scripts/tool.ts` and `src/pages/index.astro`), louder than the bordered sourcing tag — solid `--derogado` fill against a bordered outline — so it reads as more urgent than a plain unverified figure. `practice.confirmed` gets a lighter outlined tag; `practice.unknown` renders nothing. No `content/*.yaml` file has `practice` data populated yet — that arrives from Alan's verification pass (G-23) — so this ships as tested rendering capability (`test/sourcing-label.test.ts`) rather than fabricated corrections.
...
**Resolved, 2026-08-24:** ready-to-port held. `content/gaps.yaml`'s six existing gaps render as a new permanent Section 4 in `src/pages/index.astro` — server-rendered, no client JS, since it doesn't depend on the reader's answers. Each gap's optional `relatesTo` resolves to its step's title and renders as a small cross-reference tag.

**G-17** — Not covered by the handoff (no note previously recorded).

**Resolved, 2026-08-24:** the research document's own compile date (`docs/research/starting-a-business-in-puerto-rico.md`: "Research date: 2026-08-06") is now the one place duplicated into code, in `src/lib/freshness.ts`, rendered in a new masthead alert strip. This also finally lands the design handoff's surviving alert — the countdown half was cut 2026-08-07 (see the amendment above), but the "most guidance online is out of date" half was never ported until now. Names the three concrete examples already documented elsewhere in this repo (Ley 113-1974 repealed 2020, the annual report dropped 2025, the 2026 Act 60 amendment) rather than a generic staleness disclaimer.

**G-18** — Not covered by the handoff (no note previously recorded).

**Resolved, 2026-08-24:** added an optional `professional: cpa | attorney` field to `stepSchema`/`incentiveSchema`/`entityFormSchema` (`src/lib/content-schema.ts`) — kept off the shared `claimFields` rather than made universal, since it's a "which decisions route past self-service" axis, not a claim-provenance one, and a municipio's tax rate isn't a decision a reader routes anywhere. Populated the three categories 0001's Agent answer contract already names for the same reason: `entity-llc-tax-classification` and the LLC row of the entity table → `cpa`, `permiso-unico` and the Ley 60 export-services incentive → `attorney`, plus `optional-tax-1022-07` → `cpa` per its own note ("run both numbers with a CPA"). Rendered as a small amber tag wherever sourcing marks already render — every placement echoes text the note already states in prose, so it's scannable, not new advice.
...
**Resolved, 2026-08-24:** not quite the free port this note predicted. The handoff's `lang`/`S` split translates directly in principle, but the handoff is a single client-rendered `Component` class re-rendering everything on `setState`, while this repo's page is mostly server-rendered static HTML with one small interactive island (0001, "zero JavaScript by default") — porting the *mechanism* meant designing a different one. Landed as two techniques, split by which parts of the page can already re-render themselves: the tool (question panel + results) is owned end-to-end by `src/scripts/tool.ts`, which already regenerates its HTML on every answer change, so a `langchange` event just makes it regenerate in the other language too — the only path that can update a `<select><option>`'s text or an `aria-label`, neither of which can hold a hidden alternate-language span. Every other static section (masthead, alert strip, diagram, entity table, gaps) renders both languages server-side as paired `[lang="es"]`/`[lang="en"]` elements, shown or hidden by one global CSS rule keyed on `<html data-lang>` — zero JS for the actual text swap. The toggle persists to `localStorage` (`pr-guia-lang`) and is applied pre-paint by an inline script in `<head>`, the same flash-prevention technique a dark-mode toggle uses. Original English translations throughout (UI chrome and ~30 diagram labels) — not sourced legal content, so no `sourcing` mark applies.
...
**Resolved, 2026-08-24:** original copy, as predicted — a dashed-border note under Section 1's dek, before the tool grid, so a reader sees the tool's scope before answering rather than discovering the limit buried in their results.
